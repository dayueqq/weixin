<?php /* Smarty version Smarty-3.0.6, created on 2013-11-28 12:06:26
         compiled from "./nameStatics.html" */ ?>
<?php /*%%SmartyHeaderCode:13016529731c2c5f070-72236648%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  ...
  'nocache_hash' => '13016529731c2c5f070-72236648',
  'function' => 
  array (
  ),
	...
	function pageQie(page){
		...
		for(var i=page*show;i<(page+1)*show&&i<dataJson.length;i++){
			str+="<tr><td class='date'>"+dataJson[i].date+"</td><td class='brand'>"+
					 ...
					 dataJson[i].willBuyTime+"</td><td class='operation'><a onclick='return confirm(\"是否删除此购车报名？\")' href='operation.php?action=delShopUser&id="+dataJson[i].user_id+"'>删除</a></td>"+
				 +"</tr>"	;
		}
		str+="</tbody>";
		$('#tbody').html(str);
	...
	function pageQie(page){
		...
		for(var i=page*show;i<(page+1)*show&&i<finalJson.length;i++){
			str+="<tr><td class='date'>"+finalJson[i].date+"</td><td class='brand'>"+
					 ...
					 finalJson[i].willBuyTime+"</td><td class='operation'><a onclick='return confirm(\"是否删除此购车报名？\")' href='operation.php?action=delShopUser&id="+finalJson[i].user_id+"'>删除</a></td>"+
				 +"</tr>"	;
		}
		str+="</tbody>";
		$('#tbody').html(str);
